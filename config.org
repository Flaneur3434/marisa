...
#+BEGIN_SRC emacs-lisp
  (popwin-mode 1)
  (push '("*ag search*" :width 0.4 :position right :stick t) popwin:special-display-config)
  (push '("*Occur*" :stick t) popwin:special-display-config)
  (push '("*eshell*" :height 0.4 :stick t) popwin:special-display-config)
...
#+BEGIN_SRC emacs-lisp
  (use-package ligature
    :load-path "~/.emacs.d/lisp/ligature"
    :config
      ...
      (add-hook 'after-make-frame-functions
                (lambda (frame)
                  ...
                  (if (display-graphic-p frame)
                      (load-theme 'xresources t)  ;; daemon graphical
                    (load-theme 'modus-operandi t)) ;; daemon terminal
