...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-c q") 'delete-frame) ; TODO 2022-01-05: Change so its dwim
  (global-set-key (kbd "C-c t") 'my-create-tags)
  (global-set-key (kbd "C-c c") 'prot-comment-timestamp-keyword)

  (global-set-key (kbd "C-x R") 'crux-rename-file-and-buffer)
  (global-set-key (kbd "C-x x") 'crux-transpose-windows)
  (global-set-key (kbd "C-x S") 'crux-sudo-edit)
  (global-set-key (kbd "C-x X") 'crux-delete-file-and-buffer)
...
#+BEGIN_SRC emacs-lisp
  (defun xah-syntax-color-hex ()
    "Syntax color text of the form è¢¬è™”#ff1100è¢¬°ﬂ and è¢¬è™”#abcè¢¬°ﬂ in current buffer.
...
#+BEGIN_SRC emacs-lisp
  ...
  (if (daemonp)
    ...
    (progn
      (load-theme 'xresources t)
      (when window-system (set-frame-size (selected-frame) 115 55))))
#+END_SRC
* Org mode
** Description
...
** Prompt
#+BEGIN_SRC emacs-lisp
  (setq eshell-prompt-regexp "^[^è¢√è™°è¢√è™≥\n]*[è¢√è™°è¢√è™≥] ")
  (setq eshell-prompt-function
        (lambda nil
          (concat
           ...
           (if (= (user-uid) 0)
               (propertize " è¢√è™° " 'face `(:foreground "#FF6666"))
           (propertize " è¢√è™≥ " 'face `(:foreground "#A6E22E"))))))
...
#+END_SRC
*** Historical
22/04/2019: This macro was provided by user [[https://gist.github.com/ld34/44d100b79964407e5ddf41035e3cd32f][ld43]] after I couldnè¢¬è™Æt figure out how to make diminish work by being at the top of the config file.
